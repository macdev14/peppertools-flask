...
def new_os():
 ...
 if request.method=='POST':
  try:
   ...
   data=dict(request.form)
   Cadastro_OS.create(Tipo=request.form['Tipo'],Numero_Os=request.form['Numero_Os'],Ferramenta=request.form['Ferramenta'],Desenho_Cliente=request.form['Desenho_Cliente'],Desenho_Pimentel=request.form['Desenho_Pimentel'],gravacao2=request.form['gravacao2'],Id_Cliente=request.form['Id_Cliente'],Data=request.form['Data'],unidade=request.form['unidade'],Prazo=request.form['Prazo'],Data_Pedido=request.form['Data_Pedido'],Data_Nf=request.form['Data_Nf'],Especificacao=request.form['Especificacao'],Quantidade=request.form['Quantidade'],Numero_Nf=request.form['Numero_Nf'],Numero_Pedido=request.form['Numero_Pedido'],STATUS=int(request.form['STATUS'],id_Linha=request.form['id_Linha']),)
...
def editEstoque(estId):
 ...
 if request.method=='GET':
  try:
   estoque=Estoque.select(Estoque,Clientes.ID,Clientes.nome).from_(Estoque,Clientes).where(Estoque.id_cliente==Clientes.ID,Estoque.ID==estId)
...
def nfform(idnota):
 ...
 if idnota!='':
  if request.method=='POST':
   ...
   try:
    ...
    notafiscal.update(numero_nf=request.form['numero_nf'],data_nf=data['data_nf'],valor_nf=data['valor_nf'],desc=request.form['desc']).where(compras.ID==idnota).execute()
 ...
 if request.method=='POST':
  ...
  notafiscal.create(numero_nf=data['numero_nf'],data_nf=data['data_nf'],valor_nf=data['valor_nf'],desc=data['desc'])
...
def pontocad(idponto):
 ...
 if idponto!='':
  ...
  if request.method=='POST':
   ...
   try:
    ...
    ponto.update(cod_func=request.form['cod_func'],data=data['data'],hora=data['hora'],desc=data['desc']).where(ponto.id==idponto).execute()
    flash('Alterado com sucesso')
   except:
 ...
 if request.method=='POST':
  ...
  ponto.create(cod_func=request.form['cod_func'],data=data['data'],hora=data['hora'],desc=request.form['desc'])
  idpontonew=ponto.select(fn.MAX(ponto.id)).scalar()
  flash('Cadastrado com sucesso')
  return redirect('/ponto/form/'+str(idpontonew))
...
def inicioProcesso():
 ...
 osproc=Historico_os.select().where((Historico_os.id_os==osid)&(Historico_os.id_proc==idproc)&(Historico_os.inicio!='')&(Historico_os.fim!=None))
 if(osproc):
...
def fimProcesso():
 ...
 if not periodo:
  periodo=1
 althistos=Historico_os.select().where((Historico_os.id_os==osid)&(Historico_os.id_proc==idproc)&(Historico_os.periodo==periodo))
 if(althistos):
